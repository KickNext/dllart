...
static int dllart_cleanup_vm(char** error_out) {
  ...
  if (!g_state.vm_initialized_by_dllart) {
    g_state.vm_initialized = false;
    g_state.vm_initialized_by_dllart = false;
    return 0;
  }

  if (g_state.api.Dart_Cleanup == NULL) {
    g_state.vm_initialized = false;
    g_state.vm_initialized_by_dllart = false;
    return dllart_set_error(
        error_out, "Cannot cleanup Dart VM: Dart_Cleanup symbol is missing");
  }

  {
    ...
    if (cleanup_error != NULL) {
      const int rc = dllart_set_error_with_context(
          error_out, "Dart_Cleanup failed", cleanup_error);
      free(cleanup_error);
      g_state.vm_initialized = false;
      g_state.vm_initialized_by_dllart = false;
      return rc;
...
static int dllart_initialize_vm(char** error_out) {
  ...
  bool vm_already_initialized = false;
  ...
  g_state.vm_initialized_by_dllart = !vm_already_initialized;
  return 0;
}
...
static void dllart_release_isolate_pool(void) {
...
static int dllart_teardown_state(char** error_out) {
  int status = 0;
  dllart_release_isolate_pool();
  if (dllart_cleanup_vm(error_out) != 0) {
    status = -1;
  }
  dllart_release_runtime_handle();
  return status;
